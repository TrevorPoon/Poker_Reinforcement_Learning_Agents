...
def plot_hand_reward_heatmap(hand_reward_stat, filename):
    # Sort hand types for consistent ordering
    hands = sorted(hand_reward_stat['hands'].keys())
    rewards = [hand_reward_stat['hands'][hand] for hand in hands]

    # Define categories for easier reading (e.g., 13x4 grid for visualization)
    pocket_pairs = [hand for hand in hands if hand[0] == hand[1]]
    suited_hands = [hand for hand in hands if 's' in hand]
    offsuit_hands = [hand for hand in hands if 'o' in hand]

    # Extract reward values for each category
    pocket_rewards = [hand_reward_stat['hands'][hand] for hand in pocket_pairs]
    suited_rewards = [hand_reward_stat['hands'][hand] for hand in suited_hands]
    offsuit_rewards = [hand_reward_stat['hands'][hand] for hand in offsuit_hands]

    # Convert rewards to arrays and reshape for the heatmap
    pocket_matrix = np.array(pocket_rewards).reshape(13, 1)
    suited_matrix = np.array(suited_rewards).reshape(13, 12)  # Example 13x12 suited grid
    offsuit_matrix = np.array(offsuit_rewards).reshape(13, 12)  # Example 13x12 offsuit grid

    # Create figure with subplots for each category
    fig, axes = plt.subplots(1, 3, figsize=(18, 6), gridspec_kw={'width_ratios': [1, 3, 3]})

    # Pocket Pairs Heatmap
    sns.heatmap(pocket_matrix, annot=True, fmt=".1f", cmap="YlGnBu", ax=axes[0],
                xticklabels=["Pocket Pairs"], yticklabels=pocket_pairs)
    axes[0].set_title("Pocket Pairs Reward")
    axes[0].set_xlabel("")
    axes[0].set_ylabel("")

    # Suited Hands Heatmap
    sns.heatmap(suited_matrix, annot=True, fmt=".1f", cmap="YlGnBu", ax=axes[1],
                xticklabels=suited_hands[:12], yticklabels=suited_hands[::12])
    axes[1].set_title("Suited Hands Reward")
    axes[1].set_xlabel("")
    axes[1].set_ylabel("")

    # Offsuit Hands Heatmap
    sns.heatmap(offsuit_matrix, annot=True, fmt=".1f", cmap="YlGnBu", ax=axes[2],
                xticklabels=offsuit_hands[:12], yticklabels=offsuit_hands[::12])
    axes[2].set_title("Offsuit Hands Reward")
    axes[2].set_xlabel("")
    axes[2].set_ylabel("")
